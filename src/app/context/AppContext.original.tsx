...
interface AppContextType {
  ...
  pagosOT: Record<string, import('../../lib/supabase').Pago[]>;
  cargarPagosOT: (ordenId: string) => Promise<void>;
  registrarPago: (ordenId: string, metodo: 'Efectivo' | 'Tarjeta' | 'QR' | 'Transferencia', monto: number, referencia?: string) => Promise<{ ok: boolean; confirmado?: boolean; error?: string }>;
  ...
  generarReporteIngresos: (fechaInicio: string, fechaFin: string) => Promise<{ periodo?: { desde: string; hasta: string }; ingresos?: any[]; totalIngresos?: number; cantidadFacturas?: number; error?: string }>;
  generarReporteProductividad: () => Promise<{ productividad?: any[]; totalOrdenesFinalizadas?: number; totalIngresoGenerado?: number; error?: string }>;
  generarReporteValorInventario: () => Promise<{ inventario?: any[]; totalRepuestos?: number; valorTotalInventario?: number; cantidadTotalUnidades?: number; error?: string }>;
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [pagosOT, setPagosOT] = useState<Record<string, import('../../lib/supabase').Pago[]>>({});
  ...
  useEffect(() => {
    ...
    // Cargar personal y catálogos
    if (currentUser.rol === 'administrador') {
      supabase.rpc('listar_personal').then(({ data, error }) => {
        if (error) {
      ...
      supabase.rpc('obtener_catalogs').then(({ data, error }) => {
        if (error) {
  ...
  const addUsuario = async (u: Omit<Usuario, 'id'>): Promise<{ ok: boolean; error?: string }> => {
    try {
      ...
      // Crear registro en personal_taller si no es cliente
      if (u.rol !== 'cliente' && newUser.id) {
        try {
          await supabase.rpc('crear_personal', {
            p_nombre: u.nombre,
            p_cargo: u.rol === 'administrador' ? 'Administrador' :
          ...
          // No fallar la creación de usuario si falla crear personal
        }
      }
  ...
  const addPersonal = async (p: Omit<PersonalTaller, 'id'>): Promise<void> => {
    try {
      const { data, error } = await supabase.rpc('crear_personal', {
        p_nombre: p.nombre, p_cargo: p.cargo, p_especialidad: p.especialidad,
        p_telefono: p.telefono, p_email: p.email, p_estado: p.estado, p_usuario_id: p.usuarioId || null,
      });
      if (error) throw error;
      if (data?.ok) {
  ...
  const updatePersonal = async (id: string, p: Partial<PersonalTaller>): Promise<void> => {
    try {
      ...
      if (!prev) return;
      const merged = { ...prev, ...p };
      const { data, error } = await supabase.rpc('actualizar_personal', {
        p_id: id, p_nombre: merged.nombre, p_cargo: merged.cargo, p_especialidad: merged.especialidad,
        p_telefono: merged.telefono, p_email: merged.email, p_estado: merged.estado, p_usuario_id: merged.usuarioId || null,
      });
      if (error) throw error;
      if (data?.ok) {
  ...
  const deletePersonal = async (id: string): Promise<void> => {
    try {
      const { data, error } = await supabase.rpc('eliminar_personal', { p_id: id });
      if (error) throw error;
      if (data?.ok) {
  ...
  const cargarPagosOT = async (ordenId: string) => {
    const { data } = await supabase.rpc('listar_pagos_ot', { p_orden_id: ordenId });
  ...
  const registrarPago = async (
    ...
    const { data, error } = await supabase.rpc('registrar_pago', {
      p_orden_id: ordenId, p_metodo: metodo, p_monto: monto, p_referencia: referencia || null,
    });
    if (error) return { ok: false, error: error.message };
    if (!data?.ok) return { ok: false, error: data?.error };
  ...
  const cargarFacturasOT = async (ordenId: string) => {
    const { data } = await supabase.rpc('listar_facturas_ot', { p_orden_id: ordenId });
  ...
  const generarFactura = async (pagoId: string): Promise<{ ok: boolean; numero?: string; subtotal?: number; iva?: number; total?: number; error?: string }> => {
    const { data, error } = await supabase.rpc('generar_factura', { p_pago_id: pagoId });
    if (error) return { ok: false, error: error.message };
    if (!data?.ok) return { ok: false, error: data?.error };
    return {
      ok: true,
  ...
  const generarHtmlFactura = async (facturaId: string): Promise<{ ok: boolean; html?: string; error?: string }> => {
    const { data, error } = await supabase.rpc('generar_html_factura', { p_factura_id: facturaId });
    if (error) return { ok: false, error: error.message };
    if (!data?.ok) return { ok: false, error: data?.error };
  ...
  const guardarUrlPdfFactura = async (facturaId: string, urlPdf: string): Promise<{ ok: boolean; error?: string }> => {
    const { data, error } = await supabase.rpc('guardar_url_pdf_factura', { p_factura_id: facturaId, p_url_pdf: urlPdf });
    if (error) return { ok: false, error: error.message };
    return data?.ok ? { ok: true } : { ok: false, error: data?.error };
  };
  ...
  const obtenerFacturaPorOrden = async (ordenId: string): Promise<{ ok?: boolean; id?: string; numero?: string; urlPdf?: string; subtotal?: number; iva?: number; total?: number; error?: string }> => {
    const { data, error } = await supabase.rpc('obtener_factura_por_orden', { p_orden_id: ordenId });
  ...
  const generarReporteIngresos = async (fechaInicio: string, fechaFin: string): Promise<{ periodo?: { desde: string; hasta: string }; ingresos?: any[]; totalIngresos?: number; cantidadFacturas?: number; error?: string }> => {
    if (currentUser?.rol !== 'administrador') {
      return { error: 'Acceso denegado. Solo administrador puede generar reportes' };
    }
    const { data, error } = await supabase.rpc('reporte_ingresos', { p_fecha_inicio: fechaInicio, p_fecha_fin: fechaFin });
    if (error) return { error: error.message };
  ...
  const generarReporteValorInventario = async (): Promise<{ inventario?: any[]; totalRepuestos?: number; valorTotalInventario?: number; cantidadTotalUnidades?: number; error?: string }> => {
    ...
    const { data, error } = await supabase.rpc('reporte_valor_inventario');
    if (error) return { error: error.message };
    return data || { error: 'Error al generar reporte' };
  };
  ...
  const updateCatalogs = async (c: Partial<Catalogs>) => {
    ...
    setCatalogs(updated);
    try {
      await supabase.rpc('guardar_catalogs', { p_clave: 'all_catalogs', p_valor: updated as any });
    } catch (err: any) {
  ...
  return (
    <AppContext.Provider value={{
      ...
      pagosOT, cargarPagosOT, registrarPago, confirmarPago,
      facturasOT, cargarFacturasOT, generarFactura, generarHtmlFactura, guardarUrlPdfFactura, obtenerFacturaPorOrden,
      generarReporteIngresos, generarReporteProductividad, generarReporteValorInventario,
    }}>
      {children}
    </AppContext.Provider>
  );
